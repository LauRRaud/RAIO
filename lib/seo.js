...
/* Payloadi upload tuleb kas objektina (depth: 1) või paljas ID-na. */
...
export async function buildPageMetadata(locale, pageKey) {
  ...
  const result = sharedMetadata({
    ...
    etPath,
    ogImage: uploadUrl(page?.shareImage) || uploadUrl(seo?.defaultShareImage)
  });
...
export async function buildProductMetadata(locale, slug) {
  ...
  return sharedMetadata({
    ...
    etPath: `/pood/${slug}`,
    ogImage: product.images?.[0] || uploadUrl(seo?.defaultShareImage)
  });
}

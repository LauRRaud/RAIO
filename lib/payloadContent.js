...
/* Alaleht /sundmused/korralda elab OMA globaalis (host-event), mitte
...
function applyHostEvent(messages, host) {
  ...
  applyImage(target, "heroImage", host.hero?.image);
  applyImage(target, "heroImageMobile", host.hero?.mobileImage);
  recordStyle(messages, "hostEventHero", host.hero);

  applyText(target, "formatsTitle", host.formats?.title);
  recordStyle(messages, "hostEventFormats", host.formats);
  ...
  recordStyle(messages, "hostEventProcess", host.process);
  ...
  recordStyle(messages, "hostEventClosing", host.closing);
